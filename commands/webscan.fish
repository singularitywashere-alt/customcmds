function webscan
    if test (count $argv) -ne 1
        echo "Usage: webscan <target-website>"
        return 1
    end

    set -g __ws_target $argv[1]
    if not string match -qr '^https?://' "$__ws_target"
        set __ws_target "https://$__ws_target"
    end

    set -g __ws_base (echo "$__ws_target" | sed -E 's#(^https?://[^/]+).*#\1#')
    set -g __ws_domain (echo "$__ws_base" | sed -E 's#^https?://##; s#/.*##')

    echo (set_color cyan)"==> WEBSCAN: $__ws_target"(set_color normal)
    ...
    if test $found -eq 0
        echo (set_color yellow)"  No accessible pages found beyond the homepage."(set_color normal)
    else
    ...
    set -e __ws_target __ws_base __ws_domain __ws_discovered __ws_checked
end
